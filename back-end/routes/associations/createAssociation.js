...
const createAssociation = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `INSERT INTO ngo (ngo_id, ngo_name, category, creation_date, description_ong, website, photo_url, location)
    ...
    res.status(201).json({
      message: "association ajouté avec succès!",
      ngo_id: insertedAssociationId,
    });
  } catch (error) {
